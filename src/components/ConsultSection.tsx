        ...
        <div className="lg:col-span-7 bg-surface-container-low p-10 border border-outline-variant/20 relative rounded-lg">
          {isSuccess ? (
            <div className="flex flex-col items-center justify-center p-12 text-center space-y-6 h-full">
              <CheckCircle className="w-16 h-16 text-primary-container" />
              <h3 className="font-headline-lg text-2xl text-on-surface">Transmission Successful</h3>
              <p className="font-body-md text-on-surface-variant max-w-sm">
                Your architecture request has been securely routed. Our engineers will review your objectives and initiate contact shortly.
              </p>
            </div>
          ) : (
            <form className="space-y-12" onSubmit={handleSubmit}>
              ...
              {error && (
                <div className="bg-error/10 border border-error/20 p-4 rounded text-error font-body-sm">
                  {error}
                </div>
              )}
              ...
              <button
                ...
                className="w-full py-6 bg-primary-container text-on-primary font-label-caps text-label-caps text-lg tracking-widest font-bold hover:brightness-110 active:scale-[0.98] transition-all uppercase rounded-none disabled:opacity-50 disabled:cursor-not-allowed"
                type="submit"
              >
                {isSubmitting ? "TRANSMITTING..." : "Submit Architecture Request"}
              </button>
            </form>
          )}
